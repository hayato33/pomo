...
export function MobileNav() {
  ...
  if (!session) return;
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-10 flex h-16 items-center justify-center border-t border-gray-200 bg-white dark:border-gray-500 dark:bg-black sm:hidden">
          ...
          .map((item) => {
            ...
            return (
              <li key={item.href} className="relative flex-1">
                ...
                >
                  {isActive && (
                    <div className="absolute inset-x-0 top-0 h-0.5 bg-gray-900 dark:bg-gray-200" />
                  )}
                  <item.icon size={24} />
